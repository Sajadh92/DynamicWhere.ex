...
> **Structs (3.4.0).** An application's own struct — a value type outside the `System` namespaces — is built with exactly what was named, and no `Id` is added: `Name.En` keeps its default when only `Name.Ar` is named. Nested structs are built level by level, and a collection inside a struct is carried. A nullable struct is named through `Value` (`"Alias.Value.Ar"`), built where it has a value and null where it has none; `"Alias.Value"` whole, or `"Alias.HasValue"` alone, leaves it unbound. A collection of structs is built element by element into a `List<T>`, any member type a `List<T>` can be assigned to, or a `T[]`; a null collection stays null, and a collection of scalars is still bound whole. Still unbound: a class navigation inside a struct (its siblings are built), a struct or an element in which nothing named can be set, a collection of nullable structs, and a path beneath a framework value type (`"CreatedAt.Year"`, `"Code.Length"`) — a typed row cannot hold the year apart from the date, and binding the date whole would return more than the path names; `SelectDynamic` carries such a path as named. On EF Core, a list of structs the source projection builds with a collection initializer (`new List<Pair> { new Pair { … } }`) cannot be selected from again: a path beneath it fails with `InvalidOperationException` ("could not be translated"), as it always did for `SelectDynamic`; a list the projection reads from a query is built as named. Until 3.4.0 the typed projection skipped every value-typed member a path went beneath and bound a collection of structs whole — see breaking points 43 and 44.
...
Only public instance properties are read, through navigations and collection elements, up to paths of four segments. Two kinds of path lie outside that walk, and since 3.3.0 both are policed; and since 3.4.0 a path beneath an application's own struct takes the struct member's policy as well:

- **Beneath a member whose type the framework declares.** `Salary.Value` and `Salary.HasValue` on a `decimal?`, `Secret.Length` on a `string`, `Born.Year` on a `DateTime`, `Bag.Count` on a dictionary, `Lines.Count` on an application's own collection class — the collection's own member, not an element's. No attribute can be placed there, so such a path takes every fragment of the member it reads, whichever provider supplied it: the deny effects per feature, the `[DwOperators]` restriction (intersected), the `[DwCost]` weight and the audited features. Not what is said to the caller about the member — the alias, the required filter (a filter on `TenantId.Value` does not satisfy a `[DwRequireWhere]` on `TenantId`), the forced scope, and the descriptive facts. A rule naming the sub-path itself still applies alongside. One feature is one feature: `[DwNoWhere] Born` refuses `WHERE Born.Year` and still allows `GROUP BY Born.Year`, and a member nothing denies is read beneath as before, so `Name.Length` still runs. Where the member is transformed there is no member beneath it to apply the chain to, so `Select`, `Group` and `Aggregate` on the path are refused. A member only a **subtype** of the navigated type declares is not such a path: it is decided by the fragments naming it, so a grant of `Zone` under a `"*"` deny does not grant what a subtype of Zone's type declares. A class navigation is still a separate field, because its members can be decorated: a denial on `Contact` leaves `Contact.Email` open. A struct is not a navigation (the last point).
- **Past the walk's depth.** `Caps.MaxNavigationDepth` defaults to 4, the depth the walk reads to, and a host may raise it; a request naming five or more segments then reached what no fragment covered. The attributes of the member at the end of such a path are read directly now — the deny family, `[DwOperators]`, the transform stages, `[DwCost]`, `[DwAudit]`, `[DwDescribe]` and allowed values — by any resolver that reads attributes, which every resolver `DwPolicy.Configure` builds does. What is declared about the queried entity itself is left out there, as it is around a cycle: `[DwAlias]`, `[DwRequireWhere]`, `[DwForceWhere]`. A transformed member there is still a member, so `Selects` naming it returns it transformed; only a grouping key and an aggregated field are refused, because a summary's own transform finds a generated row's columns by the type's list, which stops at four segments. Since 3.4.0 the attributes of each application struct such a path passes through are read the same way.
- **Beneath a member holding an application's own struct** *(3.4.0)*. A struct — a value type outside the `System` namespaces — is a value, not a navigation, so a path beneath a member whose type is one, or a collection of them, takes that member's policy beside its own: with `[DwDenied]` on an `Iban` struct, `Iban.Number` is refused as `Iban` is. Taken from the struct member, from every provider: its deny effects per feature, its `[DwOperators]` restriction (intersected with the path's own), its `[DwCost]` weight and its audited features. Not taken: its alias, required filter, forced scope and descriptive facts, which come from the path's own member. The path's own attributes still apply (`[DwNoOrder]` on `LocalizedText.En`), and precedence decides as usual: a rule allowing `Soft.Number` replaces an overridable deny on `Soft`, and no rule replaces a sealed `[DwDenied]` on the struct. Every struct on the path decides it (`Card.Inner.Ar` takes `Card`'s and `Inner`'s), and a struct inside a class navigation counts (`Owner.Name.Ar` takes `Owner.Name`'s), where the class itself does not. A struct member that is itself transformed refuses `Select`, `Group` and `Aggregate` on its parts. `[DwNoWhere, DwNoOrder] public LocalizedText Name` refuses `Name.Ar` and `Name.En` as filters and, under `Strict`, as sorts, and both can still be selected. A member declared as a nullable struct (`Iban?`) is covered too: a query names its parts through `Value` (`Iban.Value.Number`), and the policy reads such a path as the attribute walk names it, dropping the `Value` — `Iban.Value.Number` is decided as `Iban.Number`, `Iban.Value` as `Iban` — and reads the nullable's own `Iban.HasValue` as the nullable member `Iban`. A framework nullable such as `Salary.Value` is unchanged. See breaking points 41 and 46.
...
| …where that `Select` builds the member, or the row, by calling an application type's constructor with arguments, `new LocalizedText(t.NameAr, t.NameEn)` (3.4.0) | the members an initializer beside the constructor binds; EF Core follows no other | Refused |
...
A member assigned through a sequence operator — `Lines = o.Lines.ToList()`, `o.Lines.Where(...).ToList()`, or a subquery building rows of its own — is left alone: what the row holds is not always what the navigation holds, and reading it as the navigation would refuse a member the row carries. An anonymous type is left alone whole: EF Core follows each of its members to its argument, so no member of such a row is refused here. A row or a member an application type's constructor builds with arguments has every member the constructor leaves unbound refused, and a clause on the member itself (3.4.0), because EF Core follows a member only through an initializer's binding: `new LocalizedText(t.NameAr, t.NameEn).Ar` cannot be translated, while `new LocalizedText { Ar = t.NameAr, En = t.NameEn }.Ar` is the column. 3.3.0 read such a projection as saying nothing about which member each value sets and refused none of them, so every clause on one went on to fail inside the provider; see breaking point 42. A projection is otherwise read only as far as its initializer can be read. An entity query names every producible member from the model; a projection names them only where each assignment is a nested initializer, a member copied from the entity, a value built and left empty, or a conditional over those — a null branch beside one of them included. A member assigned nothing but a null is left alone, like any assignment this shape cannot read. Past `MaxComplexDepth` — eight levels — it stops reading and stops speaking. Under `Strict` such a path still reaches the provider and still fails there, exactly as it did before 3.3.0.
...
- A narrowing that cannot be built as it was gated is refused in both tiers (3.2.0). The core's typed projection adds the key, `Id`, of every nested class it builds (never of a struct, which it builds with exactly what was named, 3.4.0), so a navigation narrowed around its own denied key would get the key back. The core reads a path only through an array, `List<T>`, `IList<T>`, `ICollection<T>`, `IEnumerable<T>`, `HashSet<T>` or `ISet<T>`, so a narrowing through any other collection fails its validation. And some members cannot be narrowed at all: a column, a complex property or a member stored as JSON, which EF Core reads whole; a member of a row in memory; and a member a projection builds some way the core cannot narrow.
...
- A navigation named through another, `Main.Lead`, gates the key of every node it passes through, which the core's projection adds, as a dotted path to a value always did (3.2.0). A denied key refuses the projection. Only a class's key: a struct's is neither added nor gated, since no builder adds it (3.4.0). Naming a field beside a denied key, `Lines.Name` when `Lines.Id` is denied, was already refused in both tiers.
...
An application's own struct holding a denied member is narrowed as a class is since 3.4.0, held directly or in a list, any type a list can be assigned to, or an array: the typed projection builds one member by member, so its allowed members come back and the denied one does not. Until then the gate read a struct as a type the core cannot build and left it out whole, with the reason `left out whole: the core cannot build '…', which it narrows into`, so its allowed members came back empty. A nullable struct, or structs in a collection of another shape, are still left out whole.
...
**A member of a struct** *(3.4.0)*. A struct is read as a copy. The compiled setter used to unbox a second copy and write into that, so a mask, a generalization, a format, a truncation, a default or a mutation declared on a member of an application's own struct — `Hash` and `Tokenize` included — landed on a temporary and the stored value was emitted: in both tiers, from rows in memory and from EF Core, typed and dynamic, whether the struct came back in a whole row or was selected whole (a dynamic selection of a path beneath it builds an object of its own, and that one was transformed already). Fixed (security): the setter writes into the box in place, and both passes of the walk write each changed struct back where it was read from, innermost first — into the member that held it, into its position in a list or an array, or into the outer struct that held it. A struct in a whole row or selected whole, nullable or not, a list or an array of structs, a struct inside a struct or inside a class navigation, and a struct past the walk's depth are all transformed, and a member the policy names is transformed once, by the pass along its path. A class member was never affected. A struct a transform changed that cannot be written back fails the query with `InvalidOperationException` rather than hand back the stored value: one held by a member with no setter, structs in a collection that cannot be written by position such as a `HashSet<T>`, and a struct that is a dictionary's value. Give such a member a setter, and hold such structs in a list or an array. A member inside a nullable struct is transformed where a selection names it through `Value` (`Pair.Value.Code`) as well: on the typed terminal, on the dynamic one, whose generated row holds the member under `Value` and which the walk steps through, as a group key (the `PairValueCode` column), and past the walk's depth, and the group floor a masked member declares applies to an aggregate over it (breaking point 46). See breaking point 45.
...
| `Purposes` | empty | Page caps per declared purpose (3.4.0), which replace `MaxPageSize` and `DefaultPageSize` for a query whose context names that purpose. See [Page caps per purpose](#page-caps-per-purpose) |
...
#### Page caps per purpose

*New in 3.4.0.* `MaxPageSize` and `DefaultPageSize` bound what one response carries, and a deployment sets them for its screens. An export or a report reads the same rows under the same field policy and was held to the same page: it stopped at the first page, or walked the rest one page at a time — a statement and a count per page, and no single snapshot of the data. `DwCaps.Purposes` gives such a read page caps of its own without raising the screens'.

```csharp
// appsettings: "Caps": { "MaxPageSize": 1000, "DefaultPageSize": 100,
//   "Purposes": { "excel": { "MaxPageSize": 10000, "DefaultPageSize": 10000 } } }
// or in code, before Configure:
...
`DwCaps.Purposes` is a `DwPurposeCaps`, an `IDictionary<string, DwPageCaps>`, and a `DwPageCaps` holds `int? MaxPageSize` and `int? DefaultPageSize`.

- A query runs under a purpose's page caps when its `DwPolicyContext.Purpose` names that purpose. The name is trimmed and compared without regard to letter case, the way a purpose-bound rule matches. A context naming no purpose, or one no entry names, runs under the deployment's caps, so a read that forgets to declare itself is bounded like a screen's.
- Since 3.4.0 `DwPolicyContext.Purpose` is stored trimmed, and a null, empty or blank value is stored as null, so `" export "` on a context means to a purpose-bound rule what it means to a purpose's caps (breaking point 40).
- A value left null takes the deployment's cap. A value set must be at least 1, or the setter throws `ArgumentOutOfRangeException`, so no purpose can switch a bound off. The default page a purpose runs under is bounded by the maximum it runs under, as the deployment's is by its own, and a purpose may lower the caps as well as raise them. With the deployment's `DefaultPageSize` at 0, a purpose that sets only `MaxPageSize` leaves a request with no page unpaged, as the deployment does.
- Only the two page caps are replaced. Every other cap applies to every purpose alike.
- A page above the purpose's maximum is refused, never trimmed: `CapExceeded`, `SourceOrigin` `"MaxPageSize cap (10000), request had 10001"`, in both tiers. It applies to `ToList` and `ToListAsync`, typed and dynamic, a `Summary`'s page, a `Segment`'s page, and the composable `Page(PageBy)`.
- Any name, and as many as a deployment has: `excel`, `csv`, `pdf`, `audit-logs`. Several names may share one `DwPageCaps` instance. A null or blank name is refused with `ArgumentException` and a null value with `ArgumentNullException`; `TryGetValue` and `ContainsKey` answer false for a null or blank name. Keys are stored trimmed.
- It binds from `Caps:Purposes:<name>:MaxPageSize` and `Caps:Purposes:<name>:DefaultPageSize` — as an environment variable, `DynamicWhere__Policies__Caps__Purposes__excel__MaxPageSize=10000` for a section at `DynamicWhere:Policies`. A misspelt key inside a purpose, or any other cap there such as `MaxConditions`, refuses to start, and so does a value below 1.
- It freezes with the posture: after startup, adding, replacing, removing or clearing a purpose, and setting a value on any `DwPageCaps` it holds, one assigned before startup included, throws `InvalidOperationException`, and `IsReadOnly` is true.
- **The purpose is the host's statement, never the caller's.** A request that could name its own purpose could name its own caps, and whatever purpose-bound grants exist. Set it in the endpoint that serves the read; never bind it from a request header or body.
...
A purpose's page caps bind the same way, `Caps:Purposes:excel:MaxPageSize` (3.4.0). Inside a purpose
only `MaxPageSize` and `DefaultPageSize` answer, so a misspelt key there, or another cap such as
`MaxConditions`, refuses to start, and so does a value below one.

Since 3.4.0 a value a property refuses is reported as the `InvalidOperationException` `Bind` documents:
the message starts `A configured policy value was refused:` and `InnerException` is the property's own
exception, however deep the binder wrapped it, a purpose's page cap inside its dictionary included. It
used to arrive as the binder's `TargetInvocationException`. See breaking point 47.
...
differently, and the second posture is refused. Purposes are compared over the names either side
declares, by the `MaxPageSize` and `DefaultPageSize` each name runs under, a value left null reading as
the deployment's own: a purpose whose caps equal the deployment's is the same posture as no entry, and
one with any other page cap is refused (3.4.0).
...
40. **A Declared Purpose Can Carry Page Caps of Its Own, and a Context Stores Its Purpose Trimmed**
    `DwCaps.MaxPageSize` and `DwCaps.DefaultPageSize` bound what one response carries, and a deployment sets them for its screens. An export or a report read the same rows under the same field policy and was held to the same page, so it stopped at the first page or walked the rest one page at a time — a statement and a count per page, and no single snapshot. Since 3.4.0 `DwCaps.Purposes` gives a declared purpose page caps of its own, and a query whose `DwPolicyContext.Purpose` names it runs under them; see [Page caps per purpose](#page-caps-per-purpose). A deployment that declares no purpose sees no change, and a second `DwPolicy.Configure` compares purposes too, by the caps that apply.

    Changed beside it: `DwPolicyContext.Purpose` is stored trimmed, and a null, empty or blank value is stored as null. A rule's purpose was already trimmed where the rule is built, so a context declaring `" export "` now matches a purpose-bound rule for `export`, which it used to miss. **Who is affected:** a host that set a purpose with surrounding spaces, whose purpose-bound rules now apply to it.

41. **The Parts of a Struct Take the Struct Member's Policy**
    A path beneath a member whose type is an application's own struct — a value type outside the `System` namespaces — or a collection of them takes that member's policy, as a path beneath a framework-typed member has since 3.3.0 (point 31). A struct is a value, not a navigation. Fixed (security) in 3.4.0, in both tiers: with `[DwDenied]` on an `Iban` struct, `Iban.Number` was filtered on — a test for a guessed value — sorted by, grouped by with its values as the group keys, and handed back by a dynamic projection, under `Strict`. Taken from the struct member, from every provider, attributes and rules alike: its denials, its operators (intersected with the path's own), its cost and its audit. Not taken: its alias, required filter, forced scope and descriptive facts, which come from the path's own member. The path's own attributes still apply beside the struct member's, and precedence decides as usual: a rule allowing `Soft.Number` replaces an overridable deny on `Soft`, and no rule replaces a sealed `[DwDenied]` on the struct. Every struct on the path decides it (`Card.Inner.Ar`), and a struct inside a class navigation counts (`Owner.Name.Ar` takes `Owner.Name`'s); the class itself does not, so a denial of `Customer` still denies only that path. A struct member that is itself transformed refuses `Select`, `Group` and `Aggregate` on its parts. Past the walk's depth the attributes of each struct the path passes through are read directly. `Explain` names the struct member's attribute as the source, and the schema reports the parts with its denials. `[DwNoWhere, DwNoOrder] LocalizedText Name` refuses `Name.Ar` and `Name.En` as filters and, under `Strict`, as sorts, and both can still be selected.

    **A nullable struct too:** its parts are named through `Value` (`Iban.Value.Number`), and since 3.4.0 the policy reads them as it names them, `Iban.Number` (point 46). **A filter on the whole struct** — a condition whose field is `Name` itself — fails in the builder with `InvalidOperationException` ("The binary operator NotEqual is not defined for the types '…' and 'System.Object'"), guarded or not, in both tiers; `[DwNoWhere]` on the member turns it into a refusal. **Who is affected:** a caller that filtered, sorted, grouped or selected a part of a denied, restricted, weighted or audited struct member is now refused, dropped, charged or recorded as for the member itself.

42. **Under `Strict`, a Path Through a Member a Constructor Builds Is Refused**
    EF Core follows a member only through an initializer's binding: `new LocalizedText(t.NameAr, t.NameEn).Ar` cannot be translated, while `new LocalizedText { Ar = t.NameAr, En = t.NameEn }.Ar` is `t.NameAr` and translates to the column. Until 3.4.0 the policy read a constructor with arguments as saying nothing about which member each value sets and refused none of them, so EF Core threw "could not be translated": a five-hundred where the strict tier promises a refusal. Under `Strict`, outside a dry run, a filter, an order, a grouping key, an aggregated field or a segment condition naming a path through such a member is now refused as an unknown name is — the clause's `FieldDeniedFor*` code, `FieldPath` `"*"` — and the trace records `the member exists on the type and the query cannot compute it, so it is refused as an unknown name is`. A row built by its own constructor, a positional record (`Select(t => new TenantRecord(t.Id, t.NameAr))`), has every member a clause names refused, and an unfiltered, unsorted read of it still works. A clause on the constructed member itself, `ORDER BY Name`, is refused too, and it can still be selected. Beside an initializer (`new Money(t.Amount) { Currency = t.Currency }`), a member the initializer binds stays usable and a member left to the constructor is refused. A constructor in either branch of a conditional refuses what that branch leaves unbound, whatever the other branch binds. Left alone, as before: an anonymous type, the framework's own types (`new DateTime(y, 1, 1)`), a collection, a type the model stores as a column (through a value converter, or a spatial point), a projection with no EF Core entity behind it, a provider that is not EF Core's own, rows in memory, `Convenience` and a dry run. `Selects` through such a member still works. **Who is affected:** no query that ran: each refused request failed inside the provider before. Build such a member with an initializer to make its parts usable. See [Blocked-action semantics](#blocked-action-semantics).

43. **A Typed Selection Through a Struct Returns Its Values**
    The typed projection — `Select<T>`, and a `Filter`'s or a `Segment`'s `Selects` on the typed terminals — skipped every value-typed member a path went beneath, so `Select(["Name.Ar"])` over a `LocalizedText` struct returned the struct's default, an empty name, with nothing refused, guarded or not. Fixed in 3.4.0: an application's own struct is built member by member, as a class navigation is, so the row carries what was named and nothing beside it. Nested structs are built level by level, a collection inside a struct is carried, and a nullable struct is named through `Value` (`Alias.Value.Ar`) and built where it has a value. Still unbound: a class navigation inside a struct, a struct in which nothing named can be set, and a path beneath a framework-typed member (`Born.Year`, `Code.Length`) — a typed row cannot hold the year apart from the date, and binding the date whole would return more than the path names; `ToListDynamic` carries it as named. A transform declared on a member of the struct is applied to the row (point 45), and a projection the policy synthesizes for a request with no `Selects` narrows a struct holding a denied member as it narrows a class, where it used to leave the struct out whole (see [A request that sends no Selects](#a-request-that-sends-no-selects)). A guarded selection through a struct carries only what was named: the gate added the `Id` of every node a selected path passes through, as the builder adds a class's key, and read a struct as one, so `DeniedBy.Value.Why` came back with the struct's `Id` beside it, typed and dynamic, and a struct whose `Id` is denied had a selection of its other members refused. Neither builder adds a struct's key, so neither does the gate. **Who is affected:** a typed query naming a path beneath a struct now receives the values instead of defaults, and a guarded selection through a struct holding an `Id` no longer receives that key unless it names it. See [Extension Methods Reference](#extension-methods-reference).

44. **A Typed Selection Beneath a Collection of Structs Carries Only What It Names**
    The typed projection bound every collection of values whole. That is right for strings, numbers and dates, but a struct has members a policy names: `Pairs.Shown` over a `List<Pair>` handed back every `Pair` whole, a `[DwDenied]` member included, in both tiers, while the policy had approved only `Pairs.Shown`. Selecting `Pairs` whole was, and is, refused when an element member is denied, and the dynamic terminals were never affected. Fixed (security) in 3.4.0, present since the policy layer shipped in 3.0.0: each element of a collection of an application's own structs is built member by member, `Pairs.Select(v => new Pair { Shown = v.Shown }).ToList()`, into a `List<T>`, any member type a `List<T>` can be assigned to, or a `T[]`. A null collection stays null. A collection of nullable structs, a collection of another shape, and an element in which nothing named can be set are left unbound: nothing is carried rather than everything. A collection of scalars is still bound whole. It works on EF Core where the projection reads the list from a query (`o.Lines.Select(l => new Pair { … }).ToList()`) and in memory.

    **Changed beside it:** a list a projection builds with a collection initializer (`Pairs = new List<Pair> { new Pair { … } }`) is one EF Core cannot select from again, so a typed selection beneath it now fails with `InvalidOperationException` ("could not be translated"), guarded or not, as the dynamic terminals always did; it used to return each element whole. Build such a list from a query, or name the collection whole where nothing in it is denied.

45. **A Transform on a Member of a Struct Is Applied**
    The outbound walk read a struct as a boxed copy, and the compiled setter unboxed a second copy to write into, so a transform declared on a member of an application's own struct — `[DwMask]` with any strategy, `Hash` and `Tokenize` included, `[DwGeneralize]`, `[DwFormat]`, `[DwTruncate]`, `[DwDefault]` and `[DwMutate]` — landed on a temporary and the stored value was emitted. Fixed (security) in 3.4.0, present since the policy layer shipped in 3.0.0: in both tiers, from rows in memory and from EF Core, typed and dynamic, whether the struct came back in a whole row or was selected whole. A class member was never affected, nor a dynamic selection of a path beneath the struct, which builds an object of its own. The setter writes into the box in place now, and both passes of the walk write each changed struct back where it was read from, innermost first: into the member that held it, into its position in a list or an array, or into the outer struct that held it. Nullable structs returned whole, lists and arrays of structs, structs inside structs, a struct inside a class navigation and a struct past the attribute walk's depth are all transformed, and a member the policy names is transformed once, by the pass along its path, never again by the pass by run-time type. A source of rows in memory is still transformed in place, its structs included, as class members are.

    **Behaviour change (fail-closed):** a struct a transform changed that cannot be written back now fails the query with `InvalidOperationException` rather than emit the stored value — a struct held by a member with no setter (the message says it passes through a struct held by a member with no setter), structs held in a collection that cannot be written by position such as a `HashSet<T>` (the message asks to hold the structs in a list or an array), and a struct that is a dictionary's value (the message says it cannot be written back where it was read from). **Who is affected:** every deployment with a transform on a struct's member now returns the transformed value where it returned the stored one, and one holding such a struct where it cannot be written back sees the query fail until the member gets a setter or the structs move into a list or an array.

46. **A Member of a Nullable Struct Is Policed as the Policy Names It**
    A query reaches a member of an `Iban?` through the nullable, `Iban.Value.Number`, because that is the member it compiles; the attribute walk looks through the nullable and names the same member `Iban.Number`, and so do the type's transforms, audits and fragments. Looked up as the query spells it, the path matched none of them. Fixed (security) in 3.4.0, present since the policy layer shipped in 3.0.0: a `[DwDenied]` on the nullable member, or on a member of the struct, did not stop a filter, a sort, a grouping key or a selection of that member, typed or dynamic, under `Strict`; `Iban.HasValue` was open; and a mask on a member of the struct was not applied when a selection named it through `Value`, as a result column or as a group key. The typed selection change of this release (point 43) had widened it to the typed terminal before release.

    A `Value` after a nullable struct of the application's is now dropped wherever the policy reads a path — the resolver, the outbound walk, the summary's key and aggregate transforms, the group floor and the past-depth transforms — so `Iban.Value.Number` is decided as `Iban.Number` and `Iban.Value` as `Iban`, and the nullable's own members, `HasValue` included, are read as the nullable member. A framework nullable, `Salary.Value`, is unchanged. With `[DwDenied] Iban? Iban`, `Iban.Value.Number` and `Iban.HasValue` are refused as `Iban` is: in every clause under `Strict`, and as a filter, a group or an aggregate under `Convenience`. A `[DwDenied]` member inside a nullable struct (`Pair.Value.Hidden`) is refused, an allowed one (`Pair.Value.Shown`) stays usable, and a mask on a member inside one (`Pair.Value.Code`) is applied on the typed terminal and on the dynamic one — the walk steps through `Value` on the generated row — as a group key (`PairValueCode`) and past the walk's depth; the group floor a masked member declares applies. `PolicyResolver.Resolve` and `Explain` report the path as the policy names it, `Iban.Number`; the trace keeps the caller's spelling. **Who is affected:** a caller that reached a denied, masked, restricted or audited member of a nullable struct through `Value` is refused, dropped, masked or recorded now, as through the member itself.
