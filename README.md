...
- **New: `DwCaps.DefaultPageSize`** (off by default) bounds a guarded query that sends no page — `MaxPageSize` only ever bounded a caller who had asked for one. **`DwCaps.MaxConditionDepth`** (default 10) bounds how deeply condition groups nest. **`DwCaps.MaxConditionSets`** (default 10) bounds how many condition sets a guarded `Segment` may carry: each set loads every row it matches before the page is cut, so the page never bounded the reads.
